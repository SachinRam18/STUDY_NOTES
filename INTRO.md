...
The problem was that traditional chatbots mainly answer predefined questions and cannot perform actions like **order tracking, refunds, or password resets**.

So, we used **RAG for accurate information retrieval** and **Agentic AI for decision-making and performing actions**.

We created a **knowledge base** containing FAQs, product manuals, support articles, and policies. These documents were **chunked, converted into embeddings, and stored in ChromaDB** for semantic search.

When a user asks a query, the system first performs **intent classification**. If it is an information-based query, **RAG retrieves relevant information from ChromaDB and passes it to the LLM**. If an action is required, **Agentic AI selects the appropriate tool or API and performs the action**.

I used **LangChain** to connect the LLM with ChromaDB and tools, and **LangGraph** to manage the multi-step workflow.

So the overall flow was:

**User Query → Intent Classification → RAG for Information / Agentic AI for Actions → Final Response**

My main contribution was implementing **RAG, ChromaDB semantic search, intent classification, and Agentic AI workflows using LangChain and LangGraph**.
